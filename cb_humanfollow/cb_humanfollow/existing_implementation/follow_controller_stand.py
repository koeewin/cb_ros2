...
class HumanPathFollowing(Node):
    ...
    def __init__(self):
        ...
        self.control = "PID"  # Options: MPC, PID, PID0
        ...
        self.numPos = 15  # Number of stored self.positions corresponding to a 1 m distance to human
        self.v_max = 0.9
    ...
    def human_position_callback(self, msg):
        ...
        if self.path_storage.shape[1] > self.numPos and (np.linalg.norm(d_rel[:2]) > 1.6 or self.total_length > 1.6):
            ...
            x_pos = self.path_storage[0,:].copy()
            y_pos = self.path_storage[1,:].copy()

            #xnew = np.linspace(np.min(x_pos), np.max(x_pos), 10)
            #xout2, yout2, weights2 = loess_1d(x_pos, y_pos, xnew, frac=0.5, degree=1)

            self.path_storage_smooth = np.row_stack((x_pos, y_pos))
        
            print(self.path_storage_smooth)
            if not np.all(np.isfinite(self.path_storage_smooth)):
                print("Found NaN or Inf in path data!")
                print(self.path_storage_smooth)
                exit()
            if self.control == "MPC":
                # Perform MPC optimization
                st = time.time()
                print(f" waypoints: need {self.optimizeProblem.N + 2}, have {self.path_storage.shape[1]}")
                if self.path_storage.shape[1] < self.optimizeProblem.N + 2:
                    print(f"Not enough waypoints: need {self.optimizeProblem.N + 2}, have {self.path_storage.shape[1]}")
                ...
                vRef = 1.5 * np.linalg.norm(self.path_storage[:, 0])  # Linear velocity [m/s]
                phi = np.arctan2(self.path_storage[1, 0], self.path_storage[0, 0])
                wRef = 0.6 * phi  # Angular velocity [rad/s]

                self.vRef = np.clip(vRef, self.v_min, self.v_max)
                self.wRef = np.clip(wRef, self.omega_min, self.omega_max)
        ...
        self.get_logger().info('Publishing: v:"%f", w:"%f"' % (float(self.vRef), float(self.wRef)))
