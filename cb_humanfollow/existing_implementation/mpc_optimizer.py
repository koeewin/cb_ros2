...
class MpcOptimizer:
    def __init__(self, n: int, t: float, u: np.ndarray, **options):
        ...
        self.VRef = options.get('VRef', 0.8)
        self.OmegaRef = options.get('OmegaRef', 0.0)
        ...
        weight_v_last = options.get('WeightVLast', 0)
        weight_omega_last = options.get('WeightOmegaLast', 0)

        self.Q = np.diag([weight_x, weight_y, weight_theta])
        self.R = np.diag([weight_v, weight_omega])
        self.S = np.diag([weight_v_last, weight_omega_last])
    ...
    def setup_problem(self):
        ...
        P = ca.SX.sym('P', (self.N+1) * (self.NumStates + self.NumControls))
    ...
    def solve_problem(self, path: np.ndarray, u_last):
        """
        ...
        self.Args['p'] = np.zeros(((self.NumStates+self.NumControls) * (self.N+1),))
        ...
        for k in range(self.N):
            ...
            #x_dot_ref = (x_next-x_ref)/(3*self.sampleTime)
            #y_dot_ref = (y_next-y_ref)/(3*self.sampleTime)

            #v_ref = np.sqrt(x_dot_ref**2+y_dot_ref**2)
            #omega_ref = theta_ref / (3*self.sampleTime)
            ...
            self.Args['p'][(self.NumStates+self.NumControls)*(k+1) : (self.NumStates+self.NumControls)*(k+1)+self.NumStates] = [x_ref, y_ref, theta_ref]
            self.Args['p'][(self.NumStates+self.NumControls)*(k+1)+3 : (self.NumStates+self.NumControls)*(k+1)+3+self.NumControls] = [v_ref, omega_ref]
    ...
    def _cost_function(self, X: ca.SX, U: ca.SX, P: ca.SX) -> ca.SX:
        ...
        for k in range(self.N):
            ...
            state_ref = P[(self.NumStates+self.NumControls)*(k+1) : (self.NumStates+self.NumControls)*(k+1)+self.NumStates]
            control_ref = P[(self.NumStates+self.NumControls)*(k+1)+3 : (self.NumStates+self.NumControls)*(k+1)+3+self.NumControls]
            if k == 0:
                control_last = P[self.NumStates : self.NumStates+self.NumControls]
            else:
                control_last = U[:, k-1]
            ...
            cost += ca.mtimes([(state - state_ref).T, self.Q, (state - state_ref)]) + \
                    ...
                    ca.mtimes([(control - control_last).T, self.S, (control - control_last)])
        return cost
